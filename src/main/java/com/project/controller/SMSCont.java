...
public class SMSCont {
    ...
    // Twilio configuration - move to env vars in production
    public static final String ACCOUNT_SID = "";
    public static final String AUTH_TOKEN = "";
    public static final String FROM_NUMBER = "";
    ...
    @PostMapping("/generateotp")
    public ResponseEntity<Map<String, String>> generateOtp(@RequestBody SMSModel smsModel) {
        String phoneNo = smsModel.getPhoneNo();
        if (!phoneNo.startsWith("+91")) {
            phoneNo = "+91" + phoneNo;
        }
        Integer otp = smsSer.generateOtp();
        boolean registered = smsSer.saveOtpToDb(phoneNo, otp);
        if (!registered) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("error", "Phone number not registered. Please register first."));
        }
        try {
            Message.creator(
    ...
    @PostMapping("/checkvalidotp")
    public ResponseEntity<Map<String, String>> verifyOtp(@RequestBody EnteringOtpModel otpRequest) {
        String phoneNo = otpRequest.getPhoneNo();
        Integer receivedOtp = otpRequest.getOtp();
        EnteringOtpModel verifiedUser = smsSer.userlogin(phoneNo, receivedOtp);
    ...
    public ResponseEntity<Map<String, String>> createOtp(@RequestBody SMSModel smsModel) {
        String phoneNo = smsModel.getPhoneNo();
        if (!phoneNo.startsWith("+91")) {
            phoneNo = "+91" + phoneNo;
        }
    ...
    public ResponseEntity<Map<String, String>> verifyOtpp(@RequestBody EnteringOtpModel otpRequest) {
        String phoneNo = otpRequest.getPhoneNo();
        Integer receivedOtp = otpRequest.getOtp();
        EnteringOtpModel verifiedUser = smsSer.userlogin(phoneNo, receivedOtp);
        if (verifiedUser != null) {
            String token = jwtService.generateToken(phoneNo, "LAB");
            Map<String, String> response = new HashMap<>();
            response.put("token", token);
            response.put("role", "LAB");
            response.put("username", phoneNo);
            return ResponseEntity.ok(response);
        } else {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
